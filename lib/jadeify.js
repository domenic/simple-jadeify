    ...
    return through(
        ...
        function () {
            ...
            readJadeifyConfigFromPackage(process.cwd(), function (err, opts) {
                ...
                var dependencies = getJadeDependencies(inputString, opts.filename, opts);
                var len = dependencies.length;
                var i;
                for (i = 0; i < len; i++) {
                  self.emit("file", dependencies[i]);
                }
...
function getJadeDependencies(inputString, filepath, opts) {
  var parser = new jade.Parser(inputString, filepath, opts);
  parser.parse();
